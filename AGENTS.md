...
이 문서는 Checky 모노레포에서 작업하는 사람과 AI agent를 위한 공통 작업 지침이다. 기존 구현을 존중하고, 작은 단위로 변경하며, 인증과 iOS 앱 경험을 먼저 안정화한다.

## 프로젝트 목표

그룹 내 구성원이 함께 사용하는 iOS 우선 Checky 앱을 만든다. 현재 우선순위는 카카오 로그인, 자체 사용자/session 구조, 반복 일정 관리와 주차 관리 기능이다.
...
- 홈 화면은 `일정 관리`, `주차 관리` 두 메뉴가 즉시 보이게 한다.
...
- 현재 주요 schema는 로그인용 `users`, `authentications`, 그룹용 `families`, `family_members`, `family_invitations`, 반복 일정과 주차 관련 schema다.
- 기능 schema와 API를 변경할 때는 `README.md`와 관련 수용 기준도 함께 갱신한다.
