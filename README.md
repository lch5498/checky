...
- 앱 내 주차 위치 저장·차량 수정/삭제 후 조회한 최신 주차 데이터는 푸시 수신이나 홈 탭 진입을 기다리지 않고 위젯에도 바로 전달합니다. 일정 데이터는 유지하고 주차 영역만 갱신합니다.
...
- iOS 주기 갱신은 앱의 `BGAppRefreshTask` 예약과 전용 Flutter 엔진을 사용합니다. Dart 준비 완료 신호 이후 조회를 시작하고, 실행 기한 만료/25초 초과 시 엔진을 정리합니다. 기존 예약이 있으면 실행 가능 시각을 뒤로 미루지 않고, 앱 활성화/백그라운드 전환 때 누락된 예약을 복구합니다. Android 주기 갱신은 WorkManager를 유지합니다.
- `설정 → 위젯 갱신`에서 실제 iOS 예약 여부, 백그라운드 새로 고침/저전력 모드, 네이티브 실행 단계, 주기/푸시 갱신 결과를 확인할 수 있습니다. `지금 갱신`은 예약을 복구하고 서버 조회를 즉시 실행합니다. `진단 정보 복사`는 시간과 상태만 복사하며 토큰·개인 데이터는 포함하지 않습니다. 수동 갱신 성공은 OS의 정기 실행 보장을 의미하지 않습니다.
